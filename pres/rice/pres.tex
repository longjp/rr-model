...
\begin{frame}{Weight Matrix}
  ...
  \item $W_{ii} = w(\sigma_{i}) = (\sigma_{i}^{2} + c)^{-1}$ (for some constant $c$)
  \end{itemize}
\end{frame}
...
\begin{frame}{Small Simulation}
  ...
  \textbf{Compare Sampling Distribution:}
  \begin{equation*}
    \widehat{\beta} = (X^T\widehat{W}X)^{-1}\widehat{W}XY
  \end{equation*}
  for $\widehat{W}=\Sigma^{-1}$, $\widehat{W} = I$, and $\widehat{W} = (\Sigma + \widehat{\Delta})^{-1}$
