...
\begin{frame}{Approximate Models with Heteroskedastic Error}
  ...
  \textbf{Question:} Does weighting observations by inverse of the variance make sense when model is an approximation?
...
\begin{frame}{Weight Matrix}
  ...
  \begin{itemize}
  \item OLS: $\widehat{W}_{ii} = w(\sigma_{i}) = 1$
  \item ``Standard'' WLS: $\widehat{W}_{ii} = w(\sigma_{i}) = \sigma_{i}^{-2}$
  \item $\widehat{W}_{ii} = (\sigma_{i}^{2} + c)^{-1}$ for any constant $c$
  \end{itemize}
\end{frame}
...
\begin{frame}{Determining the Optimal Weight Matrix}
