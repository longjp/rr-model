...
m_shift <- m - betasM
metal <- rep(0,nrow(betasM))
lzs <- seq(-5,0,.01)
for(ii in 1:nrow(m)){
    print(ii)
    rss <- rep(0,length(lzs))
    for(jj in 1:length(lzs)){
        lz <- lzs[jj]
        temp <- m_shift[ii,] - z1*(lz + 2.85) - z2*(lz+2.85)^2
        rss[jj] <- sum(lm(temp~dust)$residuals^2)
    }
    metal[ii] <- lzs[which.min(rss)]
    temp <- m_shift[ii,] - z1*(metal[ii] + 2.85) - z2*(metal[ii]+2.85)^2
    lm.fit <- lm(temp~dust)
...
dev.new()
par(mar=c(5,5,1,1))
plot(dust_comp$dust,dust['r']*dust_comp$my_dust,ylim=lim,xlim=lim,xlab="Schlegel Dust r",ylab="Model Dust r No Metal",
     cex.lab=1.3)
abline(a=0,b=1)
##dev.off()
